...
void PersWiFiManager::_awaitWiFi() {
  ...
  if (_onReadyHandler) _onReadyHandler(PWM_WIFI_AWAIT);
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (_onReadyHandler) _onReadyHandler(PWM_WIFI_CONNECTED);
...
void PersWiFiManager::_awaitSmartConfig() {
  if (WiFi.status() != WL_CONNECTED) {
    ...
    if (_onReadyHandler) _onReadyHandler(PWM_SMARTCONFIG_AWAIT);
